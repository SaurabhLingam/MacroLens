  ...
  const FemaleFigure = ({ heightPct }) => {
    const imgH = 180 + heightPct * 80;
    const imgW = imgH * 0.52;
    return (
      <Image
        source={require("../../assets/female_height.png")}
  ...
  const MaleFigure = ({ heightPct }) => {
    ...
    return (
      <Image
        source={require("../../assets/male_height.png")}
        style={{ width: imgW, height: imgH, resizeMode: "contain" }}
      />
    );
  };
      ...
      <Svg width={DIAL_W} height={DIAL_H}>
        ...
        {ticks.map((t) => (
          <Line key={t.kg}
            ...
            stroke={t.isMajor ? PURPLE_DARK : PURPLE_DARK}
  ...
  const AgeSlider = ({ value, min, max, onChange }) => {
    ...
    const pan = useRef(PanResponder.create({
      ...
      onPanResponderGrant: (e) => updateFromX(e.nativeEvent.pageX - trackLeft.current),
      onPanResponderMove:  (e) => updateFromX(e.nativeEvent.pageX - trackLeft.current),
    })).current;
    const updateFromX = (x) => {
  ...
  const CalorieCalculator = () => {
    ...
    useEffect(() => {
      const loadSaved = async () => {
        const calorieData = await AsyncStorage.getItem("calculatedCaloriesData");
        if (calorieData) setCalories(JSON.parse(calorieData));
        const settingsData = await AsyncStorage.getItem("calculatorSettings");
        if (settingsData) {
          const s = JSON.parse(settingsData);
          if (s.gender)   setGender(s.gender);
          if (s.weightKg) setWeightKgState(s.weightKg);
          if (s.weightKg) animatedKg.setValue(s.weightKg);
          if (s.heightIn) setHeightIn(s.heightIn);
          if (s.ageVal)   setAgeVal(s.ageVal);
          if (s.activity !== undefined) setActivity(s.activity);
        }
      };
      loadSaved();
    }, []);
    ...
    const calculate = async () => {
      ...
      await AsyncStorage.setItem("calculatorSettings", JSON.stringify({
        gender, weightKg, heightIn, ageVal, activity,
      }));
